...
Examples:
  ...
  python main.py --count            # Show only repository count
    ...
    parser.add_argument(
        "--repo", type=str, help="Get details for a specific repository"
    )

    parser.add_argument(
        "--count", action="store_true", help="Show only the total count of repositories"
    )

    parser.add_argument(
        "--full-agent", action="store_true", help="Run the full GitHub agent pipeline"
    )

    parser.add_argument(
        "--schedule", action="store_true", help="Run the agent scheduler continuously"
    )
...
def print_analysis_results(analyzer: RepositoryAnalyzer) -> None:
    ...
    try:
        ...
        for i, repo in enumerate(repositories, 1):
            ...
            print(
                f"     Size: {repo.get('size', 0)} KB | Stars: {repo['stars']} | Forks: {repo['forks']}"
            )
